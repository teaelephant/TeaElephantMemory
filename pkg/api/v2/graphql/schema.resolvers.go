...
func (r *queryResolver) TeaOfTheDay(ctx context.Context) (*model.TeaOfTheDay, error) {
	...
	for _, c := range cols {
		records, err := r.collectionManager.ListRecords(ctx, uuid.UUID(c.ID), user.ID)
		if err != nil {
			return nil, err
		}
		for _, rec := range records {
			...
			if _, ok := teas[id]; !ok {
				...
				if rec.ExpirationDate.Before(earlyExp[id]) {
					earlyExp[id] = rec.ExpirationDate
				}
			}
		}
	...
	w, _ := r.weather.CurrentCyprus(ctx)
	...
	_ = r.consumption.Record(ctx, user.ID, bestID, now)

	return &model.TeaOfTheDay{Tea: best, Date: now}, nil
}
